...
import { calculateSlaTimeDiff } from '../utils/slaTimeUtils';
...
export const IssueDetailPage: React.FC = () => {
  ...
  return (
    <div className="page-wrapper-narrow">
      ...
      {/* SLA Forecast Card */}
      {issue.sla && (
        <div className="glass-card mb-4" style={{ border: issue.sla.isBreached ? '2px solid var(--color-danger)' : '1px solid var(--color-slate-200)' }}>
          <div className="d-flex justify-between align-center flex-wrap gap-2 mb-4">
            <h3 style={{ fontSize: '1.125rem', fontWeight: 700, color: 'var(--color-slate-900)', margin: 0 }}>
              ⏱️ SLA Resolution Forecast
            </h3>
            {issue.sla.isBreached ? (
              <span style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '1rem' }}>
            {/* Forecasted Hours */}
            <div style={{ background: 'var(--color-slate-50)', padding: '1rem', borderRadius: 'var(--radius-md)', textAlign: 'center' }}>
              <span style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--color-primary)', display: 'block' }}>
                {issue.sla.forecastedHours}h
              </span>
            ...
            {/* Expected Resolution */}
            <div style={{ background: 'var(--color-slate-50)', padding: '1rem', borderRadius: 'var(--radius-md)', textAlign: 'center' }}>
              <span style={{ fontSize: '0.875rem', fontWeight: 700, color: issue.sla.isBreached ? 'var(--color-danger)' : 'var(--color-slate-900)', display: 'block' }}>
                {new Date(issue.sla.expectedResolutionDate).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
              </span>
              <span style={{ fontSize: '0.75rem', color: 'var(--color-slate-500)', fontWeight: 600 }}>
                Expected By
              </span>
            ...
            <div style={{ background: issue.sla.isBreached ? 'rgba(239,68,68,0.1)' : 'rgba(34,197,94,0.1)', padding: '1rem', borderRadius: 'var(--radius-md)', textAlign: 'center' }}>
              {(() => {
                const slaTime = calculateSlaTimeDiff(issue.sla.expectedResolutionDate, issue.sla.isBreached);
                return (
                  <>
                    <span style={{ fontSize: '1rem', fontWeight: 800, color: slaTime.isOverdue ? 'var(--color-danger)' : 'var(--color-success)', display: 'block' }}>
                      {slaTime.label}
                    </span>
                    <span style={{ fontSize: '0.75rem', color: 'var(--color-slate-500)', fontWeight: 600 }}>
                      {slaTime.isOverdue ? 'Overdue' : 'Time Remaining'}
                    </span>
                  </>
                );
              })()}
            ...
            {/* Escalation Level */}
            {issue.sla.escalationLevel > 0 && (
              <div style={{ background: 'rgba(245,158,11,0.1)', padding: '1rem', borderRadius: 'var(--radius-md)', textAlign: 'center' }}>
                <span style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--color-warning-dark)', display: 'block' }}>
          ...
          {issue.sla.breachedAt && (
            <p style={{ fontSize: '0.75rem', color: 'var(--color-danger)', marginTop: '0.75rem', fontWeight: 600 }}>
              ⚠ Breached at {new Date(issue.sla.breachedAt).toLocaleString()} — Priority escalated to CRITICAL
            </p>
          )}
        </div>
      )}
