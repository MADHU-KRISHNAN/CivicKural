          ...
          {activeTasks.length === 0 ? (
            ...
            activeTasks.map(task => (
              ...
              >
                ...
                <div style={{ fontSize: '13px', color: 'var(--color-slate-500)', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                ...
                {task.sla && (
                  <div style={{
                    ...
                    borderRadius: '8px',
                    marginBottom: '12px',
                    background: task.sla.isBreached ? 'rgba(239,68,68,0.08)' : 'rgba(37,99,235,0.06)',
                    border: `1px solid ${task.sla.isBreached ? 'rgba(239,68,68,0.2)' : 'rgba(37,99,235,0.15)'}`
                  }}>
                    <span style={{ fontSize: '16px' }}>{task.sla.isBreached ? '🚨' : '⏱️'}</span>
                    <div style={{ flex: 1 }}>
                      <span style={{
                        fontSize: '12px',
                        fontWeight: 700,
                        color: task.sla.isBreached ? '#dc2626' : 'var(--color-primary)',
                        display: 'block'
                      }}>
                        {task.sla.isBreached ? 'SLA BREACHED' : 'SLA Deadline'}
                      </span>
                      <span style={{ fontSize: '11px', color: 'var(--color-slate-500)' }}>
                        {new Date(task.sla.expectedResolutionDate).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                        {' — '}
                        {calculateSlaTimeDiff(task.sla.expectedResolutionDate, task.sla.isBreached).label}
                      </span>
                    </div>
                    {task.sla.escalationLevel > 0 && (
                      <span style={{
                        background: '#f59e0b',
                        color: '#fff',
                        padding: '2px 8px',
                        ...
                        ESC L{task.sla.escalationLevel}
                      </span>
                    )}
                  </div>
                )}
