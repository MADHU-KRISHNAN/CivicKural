...
class ApiService {
  ...
  async updateLocation(latitude: number, longitude: number, formattedAddress?: string, feedRadiusKm?: number): Promise<ApiResponse<User>> {
    const response = await this.makeRequest<any>('/auth/profile/location', {
      method: 'PUT',
      body: JSON.stringify({ latitude, longitude, formattedAddress, feedRadiusKm })
    });
    ...
    return { success: false, error: response.error || 'Failed to update location' };
  }
  ...
  async getIssues(userId?: string, userType?: string): Promise<ApiResponse<Issue[]>> {
    ...
    if (response.success && (response.reports || response.data)) {
      ...
      const issues: Issue[] = rawList.map((r: any) => ({
        ...
        sla: r.sla ? {
          forecastedHours: r.sla.forecastedHours,
          expectedResolutionDate: r.sla.expectedResolutionDate || new Date(new Date(r.createdAt).getTime() + (r.sla.forecastedHours * 3600 * 1000)).toISOString(),
          isBreached: r.sla.isBreached || false,
          breachedAt: r.sla.breachedAt,
          escalationLevel: r.sla.escalationLevel || 0
        } : undefined,
  ...
  async getNearbyIssues(lat?: number, lng?: number, radiusKm?: number): Promise<ApiResponse<Issue[]>> {
    const params = new URLSearchParams();
    if (lat !== undefined) params.append('lat', lat.toString());
    if (lng !== undefined) params.append('lng', lng.toString());
    if (radiusKm !== undefined) params.append('radiusKm', radiusKm.toString());
    
    const response = await this.makeRequest<any>(`/reports/nearby?${params.toString()}`);
    if (response.success && (response.reports || response.data)) {
      const rawList = response.reports || response.data;
      const issues: Issue[] = rawList.map((r: any) => ({
        id: r.id || r._id || r.reportId,
        ...
        assignedDepartment: r.assignedDepartment || r.tier2,
        createdAt: r.createdAt || new Date().toISOString(),
        updatedAt: r.updatedAt || new Date().toISOString(),
    ...
    return { success: false, error: response.error || 'Failed to load nearby issues', data: [] };
  }
  ...
  async upvoteIssue(issueId: string): Promise<ApiResponse<Issue>> {
    const response = await this.makeRequest<any>(`/reports/${issueId}/upvote`, {
      method: 'POST'
    });
    if (response.success && response.report) {
      return { success: true, data: response.report };
    }
    return { success: false, error: response.error || 'Failed to upvote' };
  }
