...
class ReportService {
  ...
  static async processReportSubmission(reportData, reqUser) {
    ...
    // === SLA Forecast Calculation ===
    try {
      const slaResult = SlaEngineService.calculateSla(report, reqUser);
      report.sla = {
      ...
      // Non-fatal: SLA calculation failure shouldn't block report creation
      console.warn('SLA calculation failed, using defaults:', slaErr.message);
    }
